...
- 范围任务续跑行为修正（解决“1-100 被裁剪为 1-80 后需重设范围”）：
  - `/import_range` 与按钮导入两条链路统一调整为：
    - 保存任务范围时保留用户目标上限（如 100），不再持久化裁剪后的 latest_seen 上限
    - 当次“监听池已观测/未观测跳过”统计仍按当前可观测上限计算
  - 效果：任务在当前源最大ID处自动完成/暂停后，后续源频道有新帖时，直接启动任务即可继续跑到原目标上限，无需重新设范围
- 过滤系统产品化调整（最小改动）：
  - 文案与逻辑对齐：
    - `仅保留纯文字` 改为 `排除纯文字`（内部仍复用 `require_text` 字段，避免破坏迁移）
    - `图片/视频` 文案明确为“必须包含图片/视频”
    - `排除链接` 语义保持为“含链接即排除”
  - 过滤面板按钮改为 emoji 状态表达（🟢/⚪️）
  - 暂时隐藏最短/最长字数在摘要与按钮入口（底层逻辑保留兼容）
  - 新增“包含关键词”能力：
    - 新字段：`task_filters.include_keywords_enabled`、`task_filters.include_keywords`
    - 规则：开启后，消息文本命中任一关键词才通过（多关键词 OR 关系）
    - 支持按钮开关与“设置关键词”输入（逗号或换行分隔）
  - `init_db` 增加旧库轻量补列迁移（task_filters 两个新字段）
- 任务详情交互文案与按钮命名优化：
  - 详情文案：`任务接收源消息` 改为 `任务监听源消息`
  - 按钮改为动态展示当前值：
    - 模式：`复制模式（当前）/转载模式（当前）`
    - 监听状态：`监听状态（开/关）`
    - 删源开关：`是否删源（是/否）`
    - 间隔：`间隔（N秒）`
    - 日上限：`日上限（N）`
  - 来源/目标编辑按钮文案改为：`来源ID修改`、`目标ID修改`
- 主菜单文案更新（按产品文案要求）：
  - `简介` 改为 `机器人简介`
  - 常用命令增加 `/start 新建任务`
  - 移除常用命令中的 `/help` 展示
  - 移除该区块中的 `/import_range` 展示，保持“新建任务”导向
- 监听源列表增强：
...
- 重写 `README.md` 为 GitHub 发布版结构（不改业务逻辑）：
  - 统一产品定位文案为“转发”语义，去除过时“搬运/轮次限制”表述
  - 重构章节为：项目定位、核心能力、运行要求、快速开始、环境变量、首次流程、完整命令清单、部署、FAQ、版本
  - 命令清单与当前代码注册能力对齐，并标注权限级别 `[A]/[S]`
  - 补充源监听池、范围任务自动完成、媒体组整体发布、关键词过滤等当前稳定能力说明
  - FAQ 增加高频问题说明（暂停后仍有日志、范围完成后的续跑、媒体组拆发排查）
- 本轮改动文件：
  - `README.md`
  - `PROJECT_LOG.md`
